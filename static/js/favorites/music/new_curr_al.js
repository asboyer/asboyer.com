...
$(document).ready(function(){
    ...
    $.getJSON(data_file, function(json) {
        ...
        $.each(albums, function(title, values){
            ...
            track_style = ""
            top_tracks = false
            if (values.tracks.length == 0) top_tracks = true;
            if(top_tracks){
            ...
            if (values.stars == null) values.stars = ""
            if (values.stars == 0.0) values.stars = ""
            var album_div =
            ...
            <div class="album_overlay">
                <div class="album-text">
                    ...
                    <p class="artist" style="${track_style}; font-size: 10px"=>${star_string}</p>
                    <div class="the-tracks" style="${track_style}">
                        <ul class="tracklist${tracklist}">
            `
            var track_list = ''
            top_tracks = values.tracks
            if (top_tracks.length > 2){
                top_tracks = top_tracks.slice(0, 3)
            }
            for (let i = 0; i < top_tracks.length; i++) {
                track_list += `
                            <li>${i + 1}. ${top_tracks[i].split("(feat")[0].split("(with")[0].split("- Including")[0]}</li>
                            `
            }
            var album_div_end = `
                        </ul>
